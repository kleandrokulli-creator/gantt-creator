...
function renderDashboard() {
  const toolbar = DOM.dashToolbar;
  const content = DOM.dashContent;
  const summary = DOM.dashSummary;
  if (!toolbar || !content) return;

  if (dashMode === 'overview') {
    _renderDashOverview(toolbar, content, summary);
  } else {
    _renderDashKanban(toolbar, content, summary);
  }
}

function _renderDashOverview(toolbar, content, summary) {
  // Toolbar
  toolbar.innerHTML = `<span style="font-weight:600;font-size:.9rem">Project Overview</span>
    <div class="dash-mode-toggle" style="margin-left:auto">
      <button class="dash-mode-btn active" onclick="setDashMode('overview')">Overview</button>
      <button class="dash-mode-btn" onclick="setDashMode('kanban')">Kanban</button>
    </div>`;
  ...
  if (l1Tasks.length === 0) {
    content.innerHTML = '<div class="org-empty">No projects found. Add Level 1 tasks in the Roadmap or Data tab.</div>';
    summary.innerHTML = '';
    return;
  }

  let html = '<div class="dash-grid">';
  let totalPct = 0, greenCount = 0, yellowCount = 0, redCount = 0;
  ...
  l1Tasks.forEach(task => {
    ...
    else if (health === 'yellow') yellowCount++;
    else if (health === 'red') redCount++;
    const hColor = HEALTH_COLORS[health];
    const childCount = allTasks.filter(t => t.outline.startsWith(task.outline + '.')).length;
    const completedCount = allTasks.filter(t => t.outline.startsWith(task.outline + '.') && t.percentComplete >= 1).length;
    const dateStr = task.start ? fmtDate(task.start) + (task.finish ? ' - ' + fmtDate(task.finish) : '') : '';
    const teamTags = (task.assigned || []).map(tn => {
      const c = TEAM_COLORS[tn] || '#64748B';
      return `<span class="tag" style="background:${c}22;color:${c};font-size:.65rem;padding:1px 5px">${esc(tn)}</span>`;
    }).join('');

    html += `<div class="dash-card" onclick="dashDrillDown(${task.id})">`;
    html += `<div class="dash-card-status" style="background:${hColor}"></div>`;
    html += `<div class="dash-card-name">${esc(task.name)}</div>`;
    html += `<div class="dash-card-meta">${dateStr}${task.duration ? ' | ' + task.duration : ''}</div>`;
    html += `<div style="display:flex;align-items:center;gap:8px;margin-top:6px">`;
    html += `<span style="font-weight:700;font-size:.85rem;color:${hColor}">${pct}%</span>`;
    html += `<div class="dash-card-bar" style="flex:1"><div class="fill" style="width:${pct}%;background:${task.color || hColor}"></div></div>`;
    html += `</div>`;
    html += `<div class="dash-card-stats">${childCount} sub-task${childCount !== 1 ? 's' : ''}${completedCount > 0 ? ' | ' + completedCount + ' completed' : ''}</div>`;
    if (teamTags) html += `<div class="dash-card-teams">${teamTags}</div>`;
    html += `</div>`;
  });
  html += '</div>';
  ...
  // Summary bar
  const avgPct = l1Tasks.length > 0 ? Math.round(totalPct / l1Tasks.length) : 0;
  summary.innerHTML = `<div class="dash-stat"><span class="dash-stat-value">${l1Tasks.length}</span> projects</div>
    <div class="dash-stat"><span class="dash-stat-value">${avgPct}%</span> avg completion</div>
    <div class="dash-stat"><span style="width:8px;height:8px;border-radius:50%;background:#16A34A;display:inline-block"></span><span class="dash-stat-value">${greenCount}</span> on track</div>
    <div class="dash-stat"><span style="width:8px;height:8px;border-radius:50%;background:#EAB308;display:inline-block"></span><span class="dash-stat-value">${yellowCount}</span> at risk</div>
    <div class="dash-stat"><span style="width:8px;height:8px;border-radius:50%;background:#DC2626;display:inline-block"></span><span class="dash-stat-value">${redCount}</span> behind</div>`;
}

function _renderDashKanban(toolbar, content, summary) {
  // Find the scoped task
  const scopeTask = dashNavStack.length > 0 ? allTasks.find(t => t.id === dashNavStack[dashNavStack.length - 1].id) : null;
  const children = scopeTask ? scopeTask.children : allTasks.filter(t => t.depth === 1);

  // Toolbar with breadcrumb
  let bcHtml = `<span class="bc-item" style="cursor:pointer;font-size:.78rem" onclick="dashBackToOverview()">Overview</span>`;
  dashNavStack.forEach((item, i) => {
    bcHtml += ` <span style="color:var(--grey-txt);margin:0 4px">></span> `;
    if (i < dashNavStack.length - 1) {
      bcHtml += `<span class="bc-item" style="cursor:pointer;font-size:.78rem" onclick="dashNavTo(${i})">${esc(item.name)}</span>`;
    } else {
      bcHtml += `<span style="font-weight:600;font-size:.78rem">${esc(item.name)}</span>`;
    }
  });

  toolbar.innerHTML = `<div style="display:flex;align-items:center;gap:8px;flex:1">${bcHtml}</div>
    <div class="dash-mode-toggle">
      <button class="dash-mode-btn" onclick="setDashMode('overview')">Overview</button>
      <button class="dash-mode-btn active" onclick="setDashMode('kanban')">Kanban</button>
    </div>`;

  // Split children into 3 columns
  const notStarted = children.filter(t => (t.percentComplete || 0) === 0 && !t.isMilestone);
  const inProgress = children.filter(t => { const p = t.percentComplete || 0; return p > 0 && p < 1 && !t.isMilestone; });
  const completed = children.filter(t => (t.percentComplete || 0) >= 1 || (t.isMilestone && t.percentComplete >= 1));

  function buildCol(title, cls, tasks) {
    let h = `<div class="kanban-col"><div class="kanban-col-header ${cls}">${title} (${tasks.length})</div><div class="kanban-cards">`;
    tasks.forEach(t => { h += _buildKanbanCard(t); });
    if (tasks.length === 0) h += `<div style="color:var(--grey-txt);font-size:.78rem;text-align:center;padding:20px">No tasks</div>`;
    h += '</div></div>';
    return h;
  }

  content.innerHTML = `<div class="kanban-wrapper">
    ${buildCol('Not Started', 'not-started', notStarted)}
    ${buildCol('In Progress', 'in-progress', inProgress)}
    ${buildCol('Completed', 'completed', completed)}
  </div>`;

  summary.innerHTML = `<div class="dash-stat"><span class="dash-stat-value">${children.length}</span> tasks</div>
    <div class="dash-stat"><span class="dash-stat-value">${completed.length}</span> completed</div>
    <div class="dash-stat"><span class="dash-stat-value">${inProgress.length}</span> in progress</div>
    <div class="dash-stat"><span class="dash-stat-value">${notStarted.length}</span> not started</div>`;
}
...
function _buildKanbanCard(task) {
  ...
  const clickAttr = hasChildren ? `onclick="dashKanbanDrillDown(${task.id})" style="cursor:pointer"` : '';
  const teamTags = (task.assigned || []).map(tn => {
    const c = TEAM_COLORS[tn] || '#64748B';
    return `<span class="tag" style="background:${c}22;color:${c};font-size:.6rem;padding:1px 4px">${esc(tn)}</span>`;
  }).join('');
  const drillHint = hasChildren ? `<span style="font-size:.65rem;color:var(--grey-txt);float:right" title="Click to drill down">${task.children.length} sub</span>` : '';

  let html = `<div class="kanban-card" ${clickAttr}>`;
  html += `<div class="kanban-card-name">${esc(task.name)}${drillHint}</div>`;
  html += `<div class="kanban-card-meta">${task.duration || ''}${pct > 0 ? ' | ' + pct + '%' : ''}</div>`;
  if (pct > 0 && pct < 100) {
    html += `<div class="kanban-card-bar"><div class="fill" style="width:${pct}%;background:${task.color || hColor}"></div></div>`;
  }
  if (teamTags) html += `<div style="margin-top:4px">${teamTags}</div>`;
  html += `</div>`;
  return html;
}
...
function setDashMode(mode) {
  dashMode = mode;
  if (mode === 'overview') { dashNavStack = []; dashScope = null; }
  renderDashboard();
}

function dashDrillDown(taskId) {
  const task = allTasks.find(t => t.id === taskId);
  if (!task) return;
  dashMode = 'kanban';
  dashNavStack = [{ id: task.id, name: task.name }];
  dashScope = taskId;
  renderDashboard();
}

function dashKanbanDrillDown(taskId) {
  const task = allTasks.find(t => t.id === taskId);
  if (!task || !task.children || task.children.length === 0) return;
  dashNavStack.push({ id: task.id, name: task.name });
  dashScope = taskId;
  renderDashboard();
}

function dashNavTo(index) {
  dashNavStack = dashNavStack.slice(0, index + 1);
  dashScope = dashNavStack[dashNavStack.length - 1].id;
  renderDashboard();
}

function dashBackToOverview() {
  dashMode = 'overview';
  dashNavStack = [];
  dashScope = null;
  renderDashboard();
